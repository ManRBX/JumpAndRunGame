...
public class BreakableBox2D : MonoBehaviour
{
    ...
    void BreakBox()
    {
        // If the box is already broken, do nothing.
        if (isBroken)
            return;

        isBroken = true;

        // Optional: Instantiate fragments (if any are assigned).
        if (fragmentsPrefabs != null && fragmentsPrefabs.Length > 0)
        {
            int index = Random.Range(0, fragmentsPrefabs.Length);
            GameObject selectedPrefab = fragmentsPrefabs[index];
        ...
        // The collider remains active – no disabling here!
    ...
    {
        // Only play the effect if the application is not quitting.
        if (isQuitting)
            return;
